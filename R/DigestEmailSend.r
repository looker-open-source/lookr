...
#' DigestEmailSend Class
#'
#' @field configuration_delivered 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite parse_json toJSON
#' @export
DigestEmailSend <- R6::R6Class(
  'DigestEmailSend',
  public = list(
    `configuration_delivered` = NULL,
    initialize = function(`configuration_delivered`){
      if (!missing(`configuration_delivered`)) {
        self$`configuration_delivered` <- `configuration_delivered`
      }
    },
    toJSON = function() {
      DigestEmailSendObject <- list()
      if (!is.null(self$`configuration_delivered`)) {
        DigestEmailSendObject[['configuration_delivered']] <- self$`configuration_delivered`
      }

      DigestEmailSendObject
    },
    fromJSONObject = function(DigestEmailSendJsonObject) {
      DigestEmailSendObject <- DigestEmailSendJsonObject #jsonlite::fromJSON(DigestEmailSendJson, simplifyVector = FALSE)
      if (!is.null(DigestEmailSendObject$`configuration_delivered`)) {
        self$`configuration_delivered` <- DigestEmailSendObject$`configuration_delivered`
      }
    },
    fromJSON = function(DigestEmailSendJson) {
      DigestEmailSendObject <- jsonlite::fromJSON(DigestEmailSendJson, simplifyVector = FALSE)
      self$fromJSONObject(DigestEmailSendObject)
    },
