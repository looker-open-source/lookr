...
QueryApi <- R6::R6Class(
  ...
  public = list(
    ...
    create_merge_query = function(body, fields, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()
	...
	if (httr::http_type(resp) == "application/json") {
	  data <- jsonlite::fromJSON(httr::content(resp, "text", encoding = "UTF-8"),simplifyVector = FALSE)
	  if ("MergeQuery" == "Character") {
	    Response$new(data, resp)
	  } else if (is.null(names(data))) {
	    returnObjects <- lapply(data, function(x) {
	      returnObject <- MergeQuery$new()
	      #returnObject$fromJSON(jsonlite::toJSON(x, auto_unbox = FALSE))
	      returnObject$fromJSONObject(x)
	      returnObject
	    })
    ...
    merge_query = function(merge_query_id, fields, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()
      ...
      urlPath <- "/merge_queries/{merge_query_id}"
      if (!missing(`merge_query_id`)) {
        urlPath <- gsub(paste0("\\{", "merge_query_id", "\\}"), `merge_query_id`, urlPath)
      }
	...
	if (httr::http_type(resp) == "application/json") {
	  data <- jsonlite::fromJSON(httr::content(resp, "text", encoding = "UTF-8"),simplifyVector = FALSE)
	  if ("TODO_OBJECT_MAPPING" == "Character") {
	    Response$new(data, resp)
	  } else if (is.null(names(data))) {
	    returnObjects <- lapply(data, function(x) {
    ...
    run_sql_query = function(slug, result_format, download, ...){
      args <- list(...)
      queryParams <- list()
      headerParams <- character()
      ...
      urlPath <- "/sql_queries/{slug}/run/{result_format}"
      if (!missing(`slug`)) {
        urlPath <- gsub(paste0("\\{", "slug", "\\}"), `slug`, urlPath)
      }
