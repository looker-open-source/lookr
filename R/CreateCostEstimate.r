...
CreateCostEstimate <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      CreateCostEstimateObject <- list()
      if (!is.null(self$`sql`)) {
        CreateCostEstimateObject[['sql']] <- self$`sql`
      }

      CreateCostEstimateObject
    },
    fromJSONObject = function(CreateCostEstimateJsonObject) {
      CreateCostEstimateObject <- CreateCostEstimateJsonObject #jsonlite::fromJSON(CreateCostEstimateJson, simplifyVector = FALSE)
      if (!is.null(CreateCostEstimateObject$`sql`)) {
        self$`sql` <- CreateCostEstimateObject$`sql`
      }
    },
    fromJSON = function(CreateCostEstimateJson) {
      CreateCostEstimateObject <- jsonlite::fromJSON(CreateCostEstimateJson, simplifyVector = FALSE)
      self$fromJSONObject(CreateCostEstimateObject)
    },
