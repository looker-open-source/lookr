...
QueryTask <- R6::R6Class(
  ...
  public = list(
	...
	if(is.null(self$`query_id`)) 'NULL' else as.character(self$`query_id`),
        self$`query`$toJSON(),
	if(is.null(self$`generate_links`)) 'NULL' else self$`generate_links`,
	if(is.null(self$`force_production`)) 'NULL' else self$`force_production`,
	if(is.null(self$`path_prefix`)) 'NULL' else self$`path_prefix`,
	if(is.null(self$`cache`)) 'NULL' else self$`cache`,
	if(is.null(self$`server_table_calcs`)) 'NULL' else self$`server_table_calcs`,
	if(is.null(self$`cache_only`)) 'NULL' else self$`cache_only`,
	if(is.null(self$`cache_key`)) 'NULL' else self$`cache_key`,
	if(is.null(self$`status`)) 'NULL' else self$`status`,
	if(is.null(self$`source`)) 'NULL' else self$`source`,
	if(is.null(self$`runtime`)) 'NULL' else as.character(self$`runtime`),
	if(is.null(self$`rebuild_pdts`)) 'NULL' else self$`rebuild_pdts`,
	if(is.null(self$`result_source`)) 'NULL' else self$`result_source`,
