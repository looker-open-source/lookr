...
UserEmailOnly <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      UserEmailOnlyObject <- list()
      if (!is.null(self$`email`)) {
        UserEmailOnlyObject[['email']] <- self$`email`
      }

      UserEmailOnlyObject
    },
    fromJSONObject = function(UserEmailOnlyJsonObject) {
      UserEmailOnlyObject <- UserEmailOnlyJsonObject #jsonlite::fromJSON(UserEmailOnlyJson, simplifyVector = FALSE)
      if (!is.null(UserEmailOnlyObject$`email`)) {
        self$`email` <- UserEmailOnlyObject$`email`
      }
    },
    fromJSON = function(UserEmailOnlyJson) {
      UserEmailOnlyObject <- jsonlite::fromJSON(UserEmailOnlyJson, simplifyVector = FALSE)
      self$fromJSONObject(UserEmailOnlyObject)
    },
