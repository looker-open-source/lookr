...
Error <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`message`, `documentation_url`){
      if (!missing(`message`)) {
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`documentation_url`)) {
        ErrorObject[['documentation_url']] <- self$`documentation_url`
      }

      ErrorObject
    },
    fromJSONObject = function(ErrorJsonObject) {
      ErrorObject <- ErrorJsonObject #jsonlite::fromJSON(ErrorJson, simplifyVector = FALSE)
    ...
    fromJSON = function(ErrorJson) {
      ...
      self$fromJSONObject(ErrorObject)
    },
