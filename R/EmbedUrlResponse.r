...
EmbedUrlResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      EmbedUrlResponseObject <- list()
      if (!is.null(self$`url`)) {
        EmbedUrlResponseObject[['url']] <- self$`url`
      }

      EmbedUrlResponseObject
    },
    fromJSONObject = function(EmbedUrlResponseJsonObject) {
      EmbedUrlResponseObject <- EmbedUrlResponseJsonObject #jsonlite::fromJSON(EmbedUrlResponseJson, simplifyVector = FALSE)
      if (!is.null(EmbedUrlResponseObject$`url`)) {
        self$`url` <- EmbedUrlResponseObject$`url`
      }
    },
    fromJSON = function(EmbedUrlResponseJson) {
      EmbedUrlResponseObject <- jsonlite::fromJSON(EmbedUrlResponseJson, simplifyVector = FALSE)
      self$fromJSONObject(EmbedUrlResponseObject)
    },
