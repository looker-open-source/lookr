...
#' @export
DigestEmails <- R6::R6Class(
  'DigestEmails',
  public = list(
    `is_enabled` = NULL,
    initialize = function(`is_enabled`){
      if (!missing(`is_enabled`)) {
        self$`is_enabled` <- `is_enabled`
      }
    },
    toJSON = function() {
      DigestEmailsObject <- list()
      if (!is.null(self$`is_enabled`)) {
        DigestEmailsObject[['is_enabled']] <- self$`is_enabled`
      }

      DigestEmailsObject
    },
    fromJSONObject = function(DigestEmailsJsonObject) {
      DigestEmailsObject <- DigestEmailsJsonObject #jsonlite::fromJSON(DigestEmailsJson, simplifyVector = FALSE)
      if (!is.null(DigestEmailsObject$`is_enabled`)) {
        self$`is_enabled` <- DigestEmailsObject$`is_enabled`
      }
    },
    fromJSON = function(DigestEmailsJson) {
      DigestEmailsObject <- jsonlite::fromJSON(DigestEmailsJson, simplifyVector = FALSE)
      self$fromJSONObject(DigestEmailsObject)
    },
